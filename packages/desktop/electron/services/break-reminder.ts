...
export interface BreakReminderSettings {
  enabled: boolean;
  intervalMinutes: number; // How often to remind (default: 60)
  minSessionMinutes: number; // Min session length before reminding (default: 30)
}
...
class BreakReminderService {
  private sessionStartTimes: Map<number, Date> = new Map(); // pid -> start time
  private lastReminderTimes: Map<number, Date> = new Map(); // pid -> last reminder
  private checkInterval: NodeJS.Timeout | null = null;
  private settings: BreakReminderSettings = DEFAULT_SETTINGS;
  ...
  loadSettings() {
    try {
      ...
      logger.info('Break reminder settings loaded:', this.settings);
    } catch (error) {
      logger.error('Failed to load break reminder settings:', error);
   ...
   * Update settings
   */
  async updateSettings(newSettings: Partial<BreakReminderSettings>) {
    try {
      const db = getDatabase();
      ...
      if (newSettings.enabled !== undefined) {
        db.prepare(
          `INSERT OR REPLACE INTO settings (key, value, updated_at) 
           VALUES ('break_reminder_enabled', ?, datetime('now'))`
        ).run(String(newSettings.enabled));
      }

      if (newSettings.intervalMinutes !== undefined) {
        db.prepare(
          `INSERT OR REPLACE INTO settings (key, value, updated_at) 
           VALUES ('break_reminder_interval', ?, datetime('now'))`
        ).run(String(newSettings.intervalMinutes));
      }

      this.settings = { ...this.settings, ...newSettings };
      logger.info('Break reminder settings updated:', this.settings);
    } catch (error) {
      logger.error('Failed to update break reminder settings:', error);
    }
  }
  ...
  stop() {
    ...
    logger.info('Break reminder service stopped');
  }
   ...
   */
  private sendBreakReminder(sessionMinutes: number) {
    try {
      ...
      logger.info(`Break reminder sent after ${timeStr}`);
    } catch (error) {
      logger.error('Failed to send break reminder:', error);
    }
  }
   ...
   */
  forceReminder(sessionMinutes: number = 60) {
    logger.info('Force reminder triggered');
    ...
    const showConfirmDialog = () => {
      ...
      }).catch((err: unknown) => logger.error('Dialog error:', err));
    };

    try {
      if (!Notification.isSupported()) {
        logger.warn('Notifications not supported, using dialog only');
        dialog.showMessageBox({
          type: 'info',
          title: 'Break Reminder Test',
