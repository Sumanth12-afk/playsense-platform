...
class AdminProtectionService {
  ...
  async initialize(): Promise<void> {
    ...
    if (stored) {
      ...
      this.isLocked = true;
      logger.info('Admin protection initialized with existing credentials');
    } else {
      ...
      try {
        ...
        if (childIdResult?.value) {
          ...
          logger.info('App configured but no admin password - forcing setup');

          // Give brief delay for window to be ready
          setTimeout(async () => {
            const credentialsSet = await this.promptSetCredentials();
            if (!credentialsSet) {
              logger.warn('User refused to set admin credentials - app will remain unprotected');
            }
          }, 2000);
        } else {
          logger.info('Admin protection waiting for initial setup');
        }
      } catch (error) {
        logger.error('Error checking child ID:', error);
      }
   ...
   */
  async promptSetCredentials(): Promise<boolean> {
    ...
    const credentials = await this.showCredentialsDialog(
      'Set Admin Credentials',
      'Create admin username and password to protect this app:',
      true // isSetup mode
    );

    if (credentials) {
      // Confirm password
      const confirmPassword = await this.showPasswordDialog(
        'Confirm Password',
      ...
      if (credentials.password === confirmPassword) {
        ...
        logger.info('Admin credentials set successfully');
        return true;
      } else {
        dialog.showMessageBox({
          type: 'error',
   ...
   */
  private async showCredentialsDialog(
    title: string,
    message: string,
    ...
    return new Promise((resolve) => {
      ...
      ipcMain.once('credentials-result', (_event: any, result: { username: string; password: string } | null) => {
        win.close();
        resolve(result);
      });
    });
   ...
   */
  private async showPasswordDialog(title: string, message: string): Promise<string | null> {
    return new Promise((resolve) => {
      const win = new BrowserWindow({
        width: 400,
        height: 250,
      ...
      const { ipcMain } = require('electron');
      ipcMain.once('password-result', (_event: any, password: string | null) => {
        win.close();
        resolve(password);
      });
    });
  ...
  async verifyAdmin(action: string): Promise<boolean> {
    ...
    if (entered && entered.username === this.adminUsername && entered.password === this.adminPassword) {
      ...
      logger.warn(`Failed admin verification for action: ${action}`);
      return false;
    }
  }
  ...
  private async storeCredentials(username: string, password: string): Promise<void> {
    ...
    try {
      ...
      await execAsync(
        `reg add "HKCU\\Software\\PlaySense\\Companion" /v AdminUsername /t REG_SZ /d "${encodedUsername}" /f`
      );
      await execAsync(
        `reg add "HKCU\\Software\\PlaySense\\Companion" /v AdminPassword /t REG_SZ /d "${encodedPassword}" /f`
      );
      // Set Protected flag - this blocks uninstall via NSIS
      await execAsync(
        `reg add "HKCU\\Software\\PlaySense\\Companion" /v Protected /t REG_SZ /d "1" /f`
      );

      logger.info('Admin credentials stored in registry with Protected flag');
    } catch (error) {
  ...
  private async getStoredCredentials(): Promise<{ username: string; password: string } | null> {
    ...
    try {
      const { stdout: usernameOutput } = await execAsync(
        'reg query "HKCU\\Software\\PlaySense\\Companion" /v AdminUsername'
      );
      const { stdout: passwordOutput } = await execAsync(
        'reg query "HKCU\\Software\\PlaySense\\Companion" /v AdminPassword'
      );
  ...
  async removeCredentials(): Promise<void> {
    ...
    try {
      ...
      logger.info('Admin credentials removed from registry');
    } catch (error) {
   ...
   */
  async setCredentialsFromUI(username: string, password: string): Promise<boolean> {
    try {
      this.adminUsername = username;
      this.adminPassword = password;
      await this.storeCredentials(username, password);
      this.isLocked = true;
      logger.info('Admin credentials set from React UI');
      return true;
    } catch (error) {
      logger.error('Failed to set credentials from UI:', error);
      return false;
    }
  }
  ...
  async canQuit(): Promise<boolean> {
    ...
    if (verified) {
      ...
      try {
        ...
        logger.info('Removed protection flag - uninstall now allowed');
      } catch (error) {
        logger.error('Failed to remove protection flag:', error);
      }
    }
   ...
   */
  async canUninstall(): Promise<boolean> {
   ...
   */
  async canChangeSettings(): Promise<boolean> {
    return await this.verifyAdmin('change settings');
  }
}

export const adminProtection = new AdminProtectionService();
